...
// camera
Camera camera(vec3(esfera_prota->centro.x, esfera_prota->centro.y,
                   esfera_prota->centro.z + 20.0f),
              vec3(0.0f, 1.0f, 0.0f), -90.0f, 0.0f);
...
void processInput(GLFWwindow *window) {
  ...
  if (glfwGetKey(window, GLFW_KEY_E) == GLFW_PRESS &&
      camera.Position.x == 0.0f && !proyectil_lanzado) {
    if (!proyectil_listo) {
      esfera_prota->calcularBoundingBox();
  ...
  // cout << "Posicion: " << camera.Position.x << " " << camera.Position.y << "
