...
public:
  ...
  GLuint setup() {
    ...
    glVertexAttribPointer(TEXCOORD0_ATTRIBUTE, 2, GL_FLOAT, GL_FALSE, 0,
                          (void *)0);
  ...
  void actualizarDatos(float t) {
    if (afectaGravedad) {
      ...
      centro.y =
          pos_ini.y + vel_ini.y * sin(radians(ang_ini)) * t - 0.5 * g * t * t;
      calcularBoundingBox();
    }
    // cout<< t << "\t" << to_string(pos_ini) << "\t" << to_string(centro) <<
    // endl;
  }
  void calcularColision(vector<Objeto *> pObjetos) {
    for (auto &obj : pObjetos) {
      if (obj != this && this->bb->Colision(*obj->bb) && obj->visible &&
          this->esPrincipal) {
